...
%Sample h(t) and x(t) until they become 0.001 of their max value.
t = 0:delta:10 - delta;
...
title("Xn plotted for until .001 of its max")
xlabel("Time")
ylabel("x(t)")
subplot(2,2,2)
plot(t(1:length(hn)), hn)
title("Hn plotted for until .001 of its max")
xlabel("Time")
ylabel("h(t)")
...
title("Convolved using discrete function vs sampling the continuous result") 
ylabel("y(t)")
xlabel("Time")
hold on
...
title("Error between signal truncated to 0.001 of max input and then convolved vs continuous convolution sampled at 50 sps")
xlabel("Time")
ylabel("Error")
